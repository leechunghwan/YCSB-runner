...
# Parse the DB string in the config, and add each DB config to the `dbs` list
for section in config.sections():
    # Load config values from this section
    # Number of times to run the workload for each MPL for the given adapter
    trials        = config.getint(section, "trials" )
    # Starting MPL (min number of YCSB threads)
    min_mpl       = config.getint(section, "min_mpl")
    # Ending MPL (maximum number of YCSB threads)
    max_mpl       = config.getint(section, "max_mpl")
    # Number by which MPL is increased for each successive trial
    inc_mpl       = config.getint(section, "inc_mpl")
    # Output format
    output        = config.get(section, "output").lower()
    # YCSB workload file to be used
    workload      = config.get(section, "workload")
...
#############################################################################
        ...
        for trial in range(1, trials + 1): # human counting (start at 1 instead of 0)
